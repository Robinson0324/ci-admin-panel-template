...
class ResearchHandler {
    ...
    public function get_research_tb(){
        ...
        $columns = array(
            ...
            array( 'db' => 'hours_per_week',      'dt' => 'hours_per_week' ),
            array( 'db' => 'additional_info',      'dt' => 'additional_info' ),
            array( 'db' => 'start_time',      'dt' => 'start_time' ),
            array( 'db' => 'end_time',      'dt' => 'end_time' ),
...
//=========date =========
        ...
        if($start_date!='')
        {
            if($filter_where!='')
                $filter_where.=' and ';
            $filter_where.=" end_time>=date('".$start_date."')";
        }
        if($end_date!='')
        {
            if($filter_where!='')
                $filter_where.=' and ';
            $filter_where.=" end_time<=date_add(date('".$end_date."'),interval 1 day)";
